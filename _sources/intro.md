...
Biological systems, computer systems, healthcare systems, solar system, and on and on ...

Systems come in different flavors and forms! Even though, the word "system" is quite familiar, it is quite hard to say precisely what it means! Loosely stated, a system is a bunch of things interacting together, somehow! The idea of a system seems quite abstract, and more often, hard to think about.  What does one do when part of a system?**Note: Sentence Incomplete** How does one act in order to change things? What causes what, when everything is interlinked? How do we know what is part of a system, and what isn't?
...
The example we have chosen is that of directed graphs: a simple language of dots and arrows between them. This is possibly the barest example we could have chosen. Yet we will see that even in this simple case, **Note: message not clear from here until the end of the sentence.** it is not clear how to think in a systemic way, with a precision that allows both a formal description with a computer, and that resolves questions about what we shall call 'dangling edges'. 

In this book, our first goal is to be able to think about directed graphs from a relational perspective. To do so, we slowly step up a four rung ladder of relational thinking. This cover the first part of the book.
